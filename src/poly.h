/** @file
 * @brief Interfejs klasy wielomianów rzadkich wielu zmiennych.
 *
...
/**
 * Górna granica typu poly_exp_t.
 */
#define POLY_EXP_T_MAX 2147483647
...
/**
 * Mnoży wielomian w miejscu przez stałą.
 * @param[in] p : wielomian @f$p@f$
...
/**
 * Zamienia wielomian na przeciwny w miejscu.
 * @param[in] p : wielomian @f$p@f$
 * @return @f$-p@f$
 */
 ...
 * W resultu może powstać wielomian, jeśli współczynniki są wielomianami.
...
/**
 * Wylicza wartość wielomianu w punkcie @p x ale niszczy 
 * strukturę podanego wielomianu.
 * @param[in] p : wielomian @f$p@f$
 * @param[in] x : wartość argumentu @f$x@f$
 * @return @f$p(x, x_0, x_1, \ldots)@f$
 */
...
/**
 * Wylicza wartość wielomianu przy podstawieniu wielomianów pod jego zmienne.
 * Dla każdego @f$i \in \{0,\cdots,k-1\}@f$ podstawia wielomian @f$q[i]@f$ pod
 * zmienną @f$x_i@f$ wielomianu @f$p@f$, przy czym zachowane jest indeksowanie
 * zmiennych w wielomianie @f$q[i]@f$. Jeżeli wielomianów podano mniej niż
 * wynosi maksymalny indeks zmiennej w @f$p@f$, to domyślnie podstawia się
 * pod te zmienne wielomiany zerowe.
 * @param[in] p : wielomian @f$p@f$
 * @param[in] k : liczba wielomianów do podstawienia
 * @param[in] q : tablica wielomianów do podstawienia
 * @return @f$p(q[0], \cdots, q[k-1])@f$
 */
Poly PolyCompose(const Poly *p, size_t k, const Poly q[]);

/**
 * Wypisuje wielomian @p p na standardowe wyjście.
 * @param[in] p : wielomian @f$p@f$
 */
...
/**
 * Wypisuje wielomian @p p na standardowe wyjście
 * z dodatkowym znakiem przejścia do nowego linea.
 * @param[in] p : wielomian @f$p@f$
 */
